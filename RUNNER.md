...
These conventions now appear consistently across `client-ana-borges-website`, `client-ps-joel`, and (partially) `client-james-young-website`. Treat them as the default for the next WordPress client project unless there's a specific reason not to. **Note a deviation**: `client-alison-avery-clinic` doesn't follow the memory-folder convention (§3 below) at all — its project memory was deliberately wiped in a "clean slate" reset and never reinstated, so it currently relies on git history alone for context. Worth reinstating there, and worth remembering this repo is the outlier if a future session goes looking for a `MEMORY.md` that isn't there.
